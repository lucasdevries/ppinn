...
class PPINN(nn.Module):
    ...
    def fit(self,
        ...
        for ep in tqdm(range(self.current_iteration + 1, self.current_iteration + epochs + 1)):
            ...
            for batch_curves, batch_coordinates, batch_collo in tqdm(dataloader):
                batch_aif = data_aif
            ...
            # check derivatis
            # derivative_coordinates = derivative_coordinates.to(self.device)
            # self.optimizer.zero_grad()
            # derivative_coordinates.requires_grad = True
            # c_aif, c_tissue, residual = self.forward_complete_check(batch_time, derivative_coordinates)
    ...
    def optimize(self,
        ...
        #     # compute data loss
        # c_aif, c_tissue, loss_residual = self.forward_complete(batch_time, batch_coordinates)
        # loss_aif, loss_tissue = self.__loss_data(batch_aif, batch_curves, c_aif, c_tissue)
        # loss += self.lw_data * (loss_aif + loss_tissue)
        # # compute residual loss
        # # c_aif, c_tissue, residual = self.forward_complete(batch_time, batch_collo)
